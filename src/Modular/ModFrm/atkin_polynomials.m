...
function AtkinModularPolynomialNormalize(Phi)
...
function AtkinModularFunctionCompute(p,prec)
...
function AtkinPowerTerms(f,j,prec)
    PS := Parent(f); q := PS.1;
    oerr := O(q^prec);
    error if true, "Not implemented error.";
end function;

function AtkinModularPolynomialCompute(p : InitialPrime := 0)
    prec := 9*p; // ???
    QQ := RationalField();
    ZZ := IntegerRing();
    P2<X,J> := PolynomialRing(ZZ,2 : Global);
    tyme := Cputime();
    f := AtkinModularFunctionCompute(p,prec);
    ...
    return AtkinModularPolynomialNormalize(F1);
end function;

intrinsic AtkinModularPolynomial(N::RngIntElt : 
    Database := true, Repository := "Echidna", Al := "Modular", InitialPrime := 0) -> RngMPolElt
	...
	if Repository eq "Echidna" then
	    ...
	    require ExistsModularCurveDatabase("Atkin") : 
		"Argument has no corresponding database entry.";
	    DBMC := ModularCurveDatabase("Atkin");
	    require N in DBMC :
		"Argument has no corresponding database entry.";
	    return Polynomial(ModularCurve(DBMC,N));
	else
	...
	return AtkinModularPolynomialCompute(N : InitialPrime := q);
    end if;
end intrinsic;

intrinsic AtkinModularFunction(N::RngIntElt,n::RngIntElt : Al := "Modular") -> RngSerLaurElt
    {}
    n := Max(n,20); 
    if Al eq "Integral" then
	return AtkinModularFunction(N,RationalField(),n);
    end if;
    DBMP := ModularPolynomialDatabase();
    if <"Atkin",N> notin DBMP then
	require N mod 12 eq 11 : "Argument 1 must be 11 mod 12.";
	return AtkinModularFunctionCompute(N,n);
    end if;
    modulus := 1;
    log_modulus := 0;
...
intrinsic AtkinModularFunction(N::RngIntElt,K::Fld,n::RngIntElt : 
    ModularPolynomial := 1, jFunctionSeries := 1) -> RngSerLaurElt
    {}
...
intrinsic ExtendAtkinModularFunction(N::RngIntElt, f::RngSerElt,
