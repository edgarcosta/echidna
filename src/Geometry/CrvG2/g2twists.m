 ...
 * Changes.
 *
 * 2007-09-26 : R. Lercier, C. Ritzenthaler, 
 *              Initial version.
 *
 ********************************************************************/
 ...
 * Bibliography
 *
 * [Mestre91] "Construction de courbes de genre 2 à partir de leurs modules"
 * Jean-François Mestre, in "Effective Methods in Algebraic Geometry",
 ...
 * intrinsic GeometricAutomorphismGroupClassification(FF::FldFin) -> GrpPerm
 *
 ********************************************************************/
...
function MestreFindPointOnConic(L) // ::RngMPolElt) 
    ...
    K := BaseRing(Parent(L));
    UP<x> := PolynomialRing(K : Global := false);
	...
	if not HasRationalPoint(C) then
  	  ...
  	  S := ReducedPoint(C);
          ...
          if S[3] eq 0 then
            pol := Evaluate(L, [UP | S[1],S[2],x]); // pol = c*x*(x-t)
            s3 := -Coefficient(pol, 1)/Coefficient(pol, 2);
            if s3 ne 0 then return S[1]/s3, S[2]/s3; end if;
            // There is only one tangent line...
            if S[1] eq 0 then
              pol := Evaluate(L, [UP | S[1]+x,S[2],x]); // pol = c*x*(x-t)
            else
              pol := Evaluate(L, [UP | S[1],S[2]+x,x]); // pol = c*x*(x-t)
            end if;
	    s3 := -Coefficient(pol, 1)/Coefficient(pol, 2);
...
G2ModelsInFF_D8 := function(GI : geometric := false)
	...
	if geometric then return [Cp]; end if;
...
G2Models := function(GI : geometric := false, models := true)
	...
	if   g1 ne g2*g3                               then
	    ...
	    aut := DirectProduct(FPGroup(CyclicGroup(2)), FPGroup(CyclicGroup(2)));
	elif g1 eq g3^3  and g2 eq g3^2 and g3 ne 0    then
	    if models then
		twists := G2ModelsInChar2FF_C2xS3(GI : geometric := geometric);
	    end if;
	    aut := DirectProduct(FPGroup(SymmetricGroup(3)), FPGroup(CyclicGroup(2)));
	    ...
	    aut := FPGroup(SmallGroup(32, 51));
	else
	    if models then
		twists := G2ModelsInChar2FF_M160(GI : geometric := geometric);
	    end if;
	    aut := FPGroup(SmallGroup(160, 235));
	end if;
	return twists, aut;
    end if;
	...
	return twists, FPGroup(SmallGroup(24,8));
    end if;
   ...
   if GI eq [ 50000, 3750, -125 ] then
	   ...
	   return twists, FPGroup(SmallGroup(240,90));
       end if;
       if models then
	   twists := G2ModelsInFF_G48(GI : geometric := geometric);
       end if;
       return twists, FPGroup(SmallGroup(48,29));
    end if;
	...
	if models then
       ...
       return twists, FPGroup(CyclicGroup(10));
   end if;
   ...
   if p eq 3 then
	   ...
	   return twists, FPGroup(DihedralGroup(6));
       end if;
   elif p eq 5 then
       /* y^2 = x^6+x^3+t */
   ...
   if p ne 5 then
	   ...
	   return twists, FPGroup(DihedralGroup(4));
       end if;
   else
       if [
	...
	return twists, DirectProduct(FPGroup(CyclicGroup(2)), FPGroup(CyclicGroup(2)));
    end if;
    
    if models then
	twists := G2ModelsInFF_C2(GI : geometric := geometric);
    end if;
    return twists, FPGroup(CyclicGroup(2));
...
intrinsic GeometricAutomorphismGroupClassification(FF::FldFin) -> GrpPerm
	...
	Grps cat:= [FPGroup(CyclicGroup(2))]; Nmbs cat:= [nbth_C2];
    end if;
    if nbth_C2xC2 ne 0 then
	Grps cat:= [DirectProduct(FPGroup(CyclicGroup(2)), FPGroup(CyclicGroup(2)))]; Nmbs cat:= [nbth_C2xC2];
    end if;
    if nbth_V4 ne 0 then
	Grps cat:= [DirectProduct(FPGroup(CyclicGroup(2)), FPGroup(CyclicGroup(2)))]; Nmbs cat:= [nbth_V4];
    end if;
    if nbth_C2xS3 ne 0 then
	Grps cat:= [DirectProduct(FPGroup(SymmetricGroup(3)), FPGroup(CyclicGroup(2)))]; Nmbs cat:= [nbth_C2xS3];
    end if;
    if nbth_D8 ne 0 then
	Grps cat:= [FPGroup(DihedralGroup(4))]; Nmbs cat:= [nbth_D8];
    end if;
    if nbth_C10 ne 0 then
	Grps cat:= [FPGroup(CyclicGroup(10))]; Nmbs cat:= [nbth_C10];
    end if;
    if nbth_D12 ne 0 then
	Grps cat:= [FPGroup(DihedralGroup(6))]; Nmbs cat:= [nbth_D12];
    end if;
    if nbth_2D12 ne 0 then
	Grps cat:= [FPGroup(SmallGroup(24,8))]; Nmbs cat:= [nbth_2D12];
    end if;
    if nbth_M32 ne 0 then
	Grps cat:= [FPGroup(SmallGroup(32, 51))]; Nmbs cat:= [nbth_M32];
    end if;
    if nbth_G48 ne 0 then
	Grps cat:= [FPGroup(SmallGroup(48,29))]; Nmbs cat:= [nbth_G48];
    end if;
    if nbth_M160 ne 0 then
	Grps cat:= [FPGroup(SmallGroup(160, 235))]; Nmbs cat:= [nbth_M160];
    end if;
    if nbth_G240 ne 0 then
	Grps cat:= [FPGroup(SmallGroup(240,90))]; Nmbs cat:= [nbth_G240];
    end if;
