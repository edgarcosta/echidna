...
intrinsic ContravariantSigmaAndPsi(Phi::RngMPolElt) -> RngMPolElt, RngMPolElt
    ...
    Psi := Evaluate(NumeratorPoly(Psi/w^(TotalDegree(Psi)-6)),XYZ_orig);
    // This definition of Sigma follows Salmon's 'symbolic notation' (a12)^4.
    Pxy<x,y> := PolynomialRing(PUVW,2);
    Rxy := Evaluate(R,[x,y,0]);
    Trans := Transvectant(Rxy,Rxy,4);
    Sigma := Trans ne 0 select (1/2)*PUVW!Coefficients(Trans)[1] else PUVW!0;
    Sigma := Evaluate(NumeratorPoly(Sigma/w^(TotalDegree(Sigma)-4)),XYZ_orig);
    return Sigma, Psi;
end intrinsic;
...
/*
intrinsic QuarticDualForm(Phi::RngMPolElt) -> RngMPolElt
    {This should give the dual curve -- a contravariant form, probably
    that called the reciprocal curve in Salmon.}
    P := Parent(Phi);
    K := BaseRing(P);
    require IsUnit(K!12) :
    ...
    Q := PolynomialRing(K,6);
    X, Y, Z, U, V, W := Explode([ Q.i : i in [1..6] ]);
    F := Evaluate(Phi,[X,Y,Z]);
    I := ideal< Q | F, U*X + V*Y + W*Z,
	U - Derivative(F,1), V - Derivative(F,2), W - Derivative(F,3) >; 
    D := Basis(EliminationIdeal(I,{U,V,W}))[1];
    return Evaluate(D,[ 0, 0, 0 ] cat [ P.i : i in [1..3] ]); 
end intrinsic;
*/
...
intrinsic DixmierInvariant(Phi::RngMPolElt,i::RngIntElt : 
    ...
    Psi := Evaluate(PUVW!NumeratorPoly(Psi/w^(TotalDegree(Psi)-6)),XYZ_orig);
    ...
    Tau := (1/12)*DifferentialOperation(Rho,Phi);
    /*
    Tau := MonomialCoefficient(Rho,u^2)*Derivative(Q,2,X)
	+ MonomialCoefficient(Rho,u*v)*Derivative(Derivative(Q,X),Y)
	+ MonomialCoefficient(Rho,u*w)*Derivative(Derivative(Q,X),Z)
	+ MonomialCoefficient(Rho,v*w)*Derivative(Derivative(Q,Y),Z)
	+ MonomialCoefficient(Rho,v^2)*Derivative(Q,2,Y)
	+ MonomialCoefficient(Rho,w^2)*Derivative(Q,2,Z);
    */
...
intrinsic DixmierInvariants(Phi::RngMPolElt : 
    ...
    */
    ...
    /*
    Rho := a0*Derivative(Psi,4,u)
	+ 4*b0*Derivative(Derivative(Psi,3,u),v)
	+ 6*c0*Derivative(Derivative(Psi,2,u),2,v)
	+ 4*d0*Derivative(Derivative(Psi,3,v),u)
	+ e0*Derivative(Psi,4,v)+4*f0*Derivative(Derivative(Psi,3,u),w)
	+ 12*g0*Derivative(Derivative(Derivative(Psi,2,u),v),w)
	+ 12*h0*Derivative(Derivative(Derivative(Psi,2,v),u),w)
	+ 4*i0*Derivative(Derivative(Psi,3,v),w)
	+ 6*j0*Derivative(Derivative(Psi,2,u),2,w)
	+ 12*k0*Derivative(Derivative(Derivative(Psi,v),u),2,w)
	+ 6*l0*Derivative(Derivative(Psi,2,w),2,v)
	+ 4*m0*Derivative(Derivative(Psi,3,w),u)
	+ 4*n0*Derivative(Derivative(Psi,3,w),v)
	+ p0*Derivative(Psi,4,w);  
    
    Tau := MonomialCoefficient(Rho,u^2)*Derivative(Phi,2,X)
	+ MonomialCoefficient(Rho,u*v)*Derivative(Derivative(Phi,X),Y)
	+ MonomialCoefficient(Rho,u*w)*Derivative(Derivative(Phi,X),Z)
	+ MonomialCoefficient(Rho,v*w)*Derivative(Derivative(Phi,Y),Z)
	+ MonomialCoefficient(Rho,v^2)*Derivative(Phi,2,Y)
	+ MonomialCoefficient(Rho,w^2)*Derivative(Phi,2,Z);
    */
    Rho := (1/144)*DifferentialOperation(Phi,Psi);
    Tau := (1/12)*DifferentialOperation(Rho,Phi);
