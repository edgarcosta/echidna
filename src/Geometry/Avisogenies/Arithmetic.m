  ...
  for i in L do
    if not IsDefined(P`prod_coord,i) then
      for t in twotorsion do
        ...
        P`prod_coord[<i[1]+t,i[2]+t>]:=P`coord[i[1]+t]*P`coord[i[2]+t];
      end for;
    end if;
  end for;
end procedure;
...
function diff_add(P,Q,PmQ,point0)
  ...
  list:={};
  for i in D do
    if PmQ`coord[i] eq 0 then 
      j:=i0;
    else j:=i; end if;
    if PmQ`coord[i] eq 0 and point0`l2  then 
  ...
  for i in D do
    if PmQ`coord[i] eq 0 then j:=i0; else j:=i; end if;
    if PmQ`coord[i] eq 0 and point0`l2 then 
  ...
  if point0`l2 then
    ...
    // we have to do it here to be sure we have computed PQ[j]
      if PmQ`coord[i] eq 0 then 
          PQ[i]+:= - PQ[j]*PmQ`coord[i]/PmQ`coord[j];
      end if;
    end for;
  end if;
...
function diff_add_PQfactor(PQ,P,Q,PmQ,point0)
  ...
  twotorsion:=point0`twotorsion;
  for i in D do
    lambda2:=&+[PQ`coord[i+t]*PmQ`coord[i+t]: t in twotorsion];
