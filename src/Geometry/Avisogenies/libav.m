...
function MyCompositeField(F1,F2: embed:=true)
  ...
  n:=GCD(n1,n2);
  FF1:=sub<F1 |n :Optimize:=false>;
  //FF2:=sub<F2 |n :Optimize:=false>;
  P1:=MinimalPolynomial(F1.1,FF1);
  //P2:=MinimalPolynomial(F2.1,FF2);
  Embed(FF1,F2);
  vprint AVIsogenies,4: "Building composite extension";
  vtime AVIsogenies,4: F0<eta>:=ext<F2 | P1: Optimize:=false>;
  F1bis<zeta>:=ext<FF1 | P1: Optimize:=false>;
  phi1:=hom<F1 -> F1bis | zeta>;
  phi2:=hom<F1bis -> F0 | eta>;
  if embed then 
    vprint AVIsogenies,4: "Embeddings";
    vtime AVIsogenies,4: Embed(F1,F1bis);
    vtime AVIsogenies,4: Embed(F1bis,F0);
  end if;
  return F0, phi1*phi2;
end function;
