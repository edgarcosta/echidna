...
import "Arithmetic.m" : diff_add_PQ, diff_add, IncludeSubgroup, \
    ...
    precompute_inverses, get_nonzero_coord, three_add;
...
/*
From the points \epslion_1 e_1 + ... + \epsilon_g e_g where the e_i are 
...
For each element i of D\subsets M_{r,g}(z/(nl)Z) we compute the premimages of 
  ...
  [i,p]     if r=2
  [i,p,p,p] if r=4
where p is usualy 0 (see bellow), by the application derived form phi:
...
function Isogenie(L,precomp: precomp_power:=true,isprefilled:=false)
  ...
  P0:=init_theta_null_point(r: init:="basic",precomp:=precomp);
  return P0, ltors;
end function;
...
function IsogenieG2(r,precomp: precomp_power:=true,
  ...
  //vtime AVIsogenies, 3: 
  newP0:=Isogenie(L,precomp: precomp_power:=precomp_power, 
