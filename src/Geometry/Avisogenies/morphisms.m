...
function IgusaTheorem(g,A,TH)
  ...
  for a in Ab do
    ...
    t*:=TH[1]`coord[Ab!htg(n1)+a];
    t*:=TH[2]`coord[Ab!htg(n2)+a];
    t*:=TH[3]`coord[Ab!htg(n3)+a];
    t*:=TH[4]`coord[Ab!htg(n4)+a];
...
function YS_fromMumford_Delta(g,a,S,points)
  ...
  K<x,y>:=PolynomialRing(Parent(points[1][1]),2);
...
// t is theta[UoB1] * theta[UoB2] * theta[UoB3] * theta[UoB4] /t_empty(z)^4
      ...
      th`coord[e]*:=sign_theta_normalized(g,U sdiff S1) / thc`coord[Ab!ep];
      ep:=htg(eta_normalized(g,U sdiff S2));
      th`coord[e]*:=sign_theta_normalized(g,U sdiff S2) / thc`coord[Ab!ep];
      ep:=htg(eta_normalized(g,{}));
      th`coord[e]*:=sign_theta_normalized(g,{})  / thc`coord[Ab!ep];
	   ...
	   ( 2*#(V meet B[4]) gt #B[4] ) then
          ...
          if #W ne 0 then
            ...
            for l in {1..(2*g+1)} do
              ...
              if l notin W then
	      if nb gt 0 then
                t/:=( (-1)^g*Evaluate(up,a[l]) )^(Integers()!(nb/2));
              end if;
              end if;
 	    end for;
...
// t is theta[UoB1] * theta[UoB2] * theta[UoB3] * theta[UoB4] /t_empty(z)^4
      ...
      // scale the theta. i.e. constants used in doubling
      ep:=htg(eta_normalized(g,U sdiff S1));
      thp`coord[e]*:=sign_theta_normalized(g,U sdiff S1) / thc`coord[Ab!ep];
      ep:=htg(eta_normalized(g,U sdiff S2));
      thp`coord[e]*:=sign_theta_normalized(g,U sdiff S2) / thc`coord[Ab!ep];
      ep:=htg(eta_normalized(g,{}));
      thp`coord[e]*:=sign_theta_normalized(g,{})  / thc`coord[Ab!ep];
