...
intrinsic ReducedForms(D::RngIntElt) -> SeqEnum 
   {A sequence of representative reduced forms for the quadratic 
   form classes of discriminant D.} 
   require D mod 4 in {0,1} : "Argument is not a discriminant";
   DK, m := FundDiscCond(D);
   if D lt 0 and m eq 1 then
      return RF(D);
   end if;
   return ReducedFormsCond(DK,m);
end intrinsic;

intrinsic ReducedForms(Q::QuadBin) -> SeqEnum 
   {A sequence of representative reduced forms for Q.} 
   D := Discriminant(Q);
   DK, m := FundDiscCond(D);
   if D lt 0 and m eq 1 then
      return RF(D);
